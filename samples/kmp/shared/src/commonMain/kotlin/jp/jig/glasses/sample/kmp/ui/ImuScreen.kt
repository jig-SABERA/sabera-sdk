    ...
    Scaffold(
        ...
        Column(
            ...
            if (data == null) {
                ...
                ImuRow(
                    "角速度 [dps]",
                    "${data.gyroXDps.oneDecimal()} / ${data.gyroYDps.oneDecimal()} / ${data.gyroZDps.oneDecimal()}",
                )
                Spacer(Modifier.height(8.dp))
                ImuRow("ピッチ [度]", data.pitchDegrees.oneDecimal())
                ImuRow("ヨー [度]", data.yawDegrees.oneDecimal())
                Spacer(Modifier.height(8.dp))
                Text(
...
private fun Float.oneDecimal(): String {
    ...
    val tenths = (abs(toDouble()) * 10).roundToLong()
    val sign = if (toRawBits() < 0) "-" else ""
