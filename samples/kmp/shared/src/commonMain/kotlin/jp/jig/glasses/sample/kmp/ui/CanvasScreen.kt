...
fun CanvasScreen(client: GlassClient, onBack: () -> Unit) {
    ...
    val used = elements.sumOf { it.text.encodeToByteArray().size + ELEMENT_OVERHEAD_BYTES }

    fun safeRun(label: String, block: () -> Unit) {
        SampleLog.d(TAG, "safeRun: $label")
        try {
            block()
        } catch (e: Throwable) {
            SampleLog.e(TAG, "safeRun: $label FAILED", e)
            error = e.message
        }
    }
    ...
    LaunchedEffect(animating) {
        ...
        val frames = try {
            withContext(Dispatchers.Default) { loadBadAppleFrames() }.also {
                require(it.isNotEmpty()) { "動画データにフレームがありません" }
            }
    ...
    val picker = rememberImagePicker(CANVAS_IMAGE_MAX_SIZE) { result ->
        if (result.exceptionOrNull() is CancellationException) return@rememberImagePicker
        safeRun("loadImage") {
            image = result.getOrThrow()
        }
    }
