...
fun BinaryVideoScreen(client: GlassClient, onBack: () -> Unit) {
    ...
    LaunchedEffect(Unit) {
        try {
            frames = withContext(Dispatchers.Default) { loadPackedFrames() }
        } catch (e: Throwable) {
            SampleLog.e(TAG, "loadPackedFrames failed", e)
...
private fun loadPackedFrames(): List<ByteArray> {
    val bytes = readSampleAsset(BUNDLED_ASSET)
